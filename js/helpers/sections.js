...
(function () {
    function displayData() {
      ...
      // Ensure the vis element starts visible for section 0, hidden otherwise
      try {
        var visStartEl = document.querySelector(cfg.visSelector);
        if (visStartEl) {
          visStartEl.classList.remove(cfg.visHiddenClass, cfg.visVisibleClass);
          if ((cfg.showAt || 0) > 0) {
            visStartEl.classList.add(cfg.visHiddenClass);
          } else {
            visStartEl.classList.add(cfg.visVisibleClass);
          }
        }
      } catch (e) { }
